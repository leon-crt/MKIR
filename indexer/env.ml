...
let declare env lc id scope st ty : unit =
 let res = _declare env lc id scope st ty in
 Indexing.DB.insert ty id ;
 (*Format.printf "@[<v>TYPE: %a@]@." Term.pp_term ty ;*)
 Format.printf "@[<v>INSERT: %a@]@." pp_ident id ;
 List.iter (fun i -> Format.printf "@[%a@]@." pp_ident i) (Indexing.DB.search ty);
 res
...
let define env lc id scope op te ty_opt : unit =
 let res = _define env lc id scope op te ty_opt in
 Option.iter (fun ty ->
 Indexing.DB.insert ty id ;
 (*Format.printf "@[<v>TYPE: %a@]@." Term.pp_term ty ;*)
 Format.printf "@[<v>INSERT: %a@]@." pp_ident id ;
